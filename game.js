...
startBtn.onclick=()=>{started=true;reset()};

let flipLock=false,shootLock=false;

function hurt(){
  if(player.inv>0||won)return;
  player.lives--;player.inv=1.25;camera.shake=22;camera.flash=.24;spawn(player.x+30,player.y+26,40,'#ff6e9d',300);updateHud();
  if(player.lives<=0){
...
function update(dt){
  ...
  if(keys.flip&&!flipLock){
    ...
    spawn(player.x+30,player.y+26,28,'#c97bff',260);
  }
  flipLock=keys.flip;
  ...
  for(const q of crystals){
    ...
    if(Math.hypot(player.x+30-q.x,player.y+26-cy)<48){
      q.taken=true;player.collected++;spawn(q.x,cy,32,'#e889ff',290);updateHud();
    }
  }
  ...
  for(const e of enemies){
    ...
    if(e.fire<=0&&Math.abs(e.x-player.x)<700){
      ...
      enemyShots.push({x:e.x+27,y:ey+23,vx:dx/len*(e.type===2?335:270),vy:dy/len*(e.type===2?335:270),life:3.2,type:e.type,trail:[]});
  ...
  for(let i=shots.length-1;i>=0;i--){
    ...
    for(const e of enemies){
      ...
      if(overlap({x:s.x-10,y:s.y-10,w:20,h:20},{x:e.x,y:ey,w:54,h:46})){
        e.hp--;e.hit=.16;spawn(s.x,s.y,20,'#f1dfd0',220);camera.shake=7;hit=true;
        if(e.hp<=0){e.alive=false;player.kills++;spawn(e.x+27,ey+23,e.type===2?64:44,e.type===2?'#ffb548':'#c36dff',330);updateHud()}
        break;
      }
    }
  ...
  if(player.x>world.width-175){
    won=true;spawn(player.x,player.y,80,'#dcb8ff',340);
